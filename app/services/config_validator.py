...
def _check_security(settings: Settings, is_prod: bool, issues: list[ConfigIssue]) -> None:
    ...
    if not settings.NEXUS_API_KEY.strip():
        issues.append(ConfigIssue(
            "error",
            "NEXUS_API_KEY must be set in production. "
    ...
    if cors_raw:
        ...
        for origin in origins:
            if origin != "*" and not origin.startswith(("http://", "https://")):
                issues.append(ConfigIssue(
                    "warning",
                    f"CORS origin '{origin}' lacks an http(s):// scheme. "
                    "Browsers require fully-qualified origins.",
                ))
                break
...
def _check_contradictions(settings: Settings, issues: list[ConfigIssue]) -> None:
    ...
    if settings.OTEL_ENABLED and settings.LOCAL_ONLY:
        issues.append(ConfigIssue(
            "warning",
            "OTEL_ENABLED=true with LOCAL_ONLY=true: the OTel exporter "
            "will fail to reach the collector endpoint.",
        ))

    if settings.WEBHOOKS_ENABLED and settings.LOCAL_ONLY:
        issues.append(ConfigIssue(
            "warning",
            "WEBHOOKS_ENABLED=true with LOCAL_ONLY=true: webhook delivery "
            "to external URLs will fail.",
        ))
...
def _check_numeric_bounds(settings: Settings, issues: list[ConfigIssue]) -> None:
    ...
    if settings.AGENT_MAX_STEPS < 1:
        issues.append(ConfigIssue(
            "warning",
            f"AGENT_MAX_STEPS={settings.AGENT_MAX_STEPS} is less than 1. "
            "Agent loop will not execute any steps.",
        ))
